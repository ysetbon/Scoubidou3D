...
const geom = await page.evaluate(() => {
  ...
  return { scale: r.width / cv.width, left: r.left, top: r.top, w: cv.width, h: cv.height };
});
const CELL = Math.max(1, Math.min(14, Math.floor((1120 - 400 - 16) / 21), Math.floor(560 / 21)));
const clickCell = async (col, row) => {
  const x = geom.left + (400 + col * CELL + CELL / 2) * geom.scale;
  const y = geom.top + (34 + row * CELL + CELL / 2) * geom.scale;
...
const cursorCell = async () => page.evaluate(({ cell }) => {
  const cv = document.querySelector('.trace-panel canvas');
  const { data } = cv.getContext('2d').getImageData(0, 0, cv.width, cv.height);
  let minX = 1e9, minY = 1e9;
  for (let y = 0; y < cv.height; y += 1) {
    for (let x = 400; x < cv.width; x += 1) {
  ...
  return { col: Math.round((minX + 1 - 400) / cell), row: Math.round((minY + 1 - 34) / cell) };
}, { cell: CELL });
...
const dimCount = () => page.evaluate(() => {
  ...
  const cv = document.querySelector('.trace-panel canvas');
  const { data } = cv.getContext('2d').getImageData(400, 34, 21 * 14, 21 * 14);
  let n = 0;
