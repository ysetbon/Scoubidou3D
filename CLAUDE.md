...
The commands, what the output means, and the diagnosis order when nothing comes
back are in **[docs/picks-shelf.md](docs/picks-shelf.md)**. Read it and hand the
PowerShell block over directly — do not ask the user to paste a token, and do not
ask which shell they use. They are on Windows PowerShell, the clone is at
`$HOME\Scoubidou3D`, and no token is needed: cache reads are public.

The one thing worth knowing before you read it, because it silently produced a
wrong answer once: **a `/cache/…` read has to gunzip.** Artifacts are stored
gzipped and the Worker serves those bytes untouched, so `Invoke-RestMethod`
returns a byte array whose `.judgements` is `$null` — which prints nothing, raises
nothing, and looks exactly like an empty shelf. `/catalogue` is ordinary JSON and
is the exception.

**This session cannot run any of it.** The sandbox's egress policy returns 403 on
CONNECT to `mxn-solutions-api.ysetbon.workers.dev` (HTTP 000 to curl). Hand the
commands over rather than offering to run them, and ask for the output back.
