...
// depths, and one of them in the other hand. Straight out of `boxStitchMN` — the
// same call the studio's own Browse grid makes, so the page cannot be showing a
// different object from the sample.
//
...
const { faces } = JSON.parse(spec) as {
  faces: Array<{ id: string; m: number; n: number; hand: Hand; rounds: number; label: string }>;
};

mkdirSync(outDir, { recursive: true });
for (const f of faces) {
  const scene = boxStitchMN(f.m, f.n, `${f.m}x${f.n} ${f.hand}`, f.hand, f.rounds);
